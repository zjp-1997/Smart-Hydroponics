...
import {
  batchDeleteSystemAnnouncements,
  deleteSystemAnnouncement,
  getSystemAnnouncementStatistics,
...
// 页面基础状态：侧栏、表格、分页和弹窗都集中声明，便于维护。
const sidebarVisible = ref(false)
const tableLoading = ref(false)
const dialogVisible = ref(false)
const selectedRows = ref<SystemAnnouncement[]>([])
const rows = ref<SystemAnnouncement[]>([])
...
// 查询表单只保留公告业务真正需要的三个条件：标题、内容、发布时间。
...
// 发布和编辑共用一个弹窗表单，后端会自动补齐发布人与接收人。

// 公告级别枚举与后端 level 字段保持一致。
...
// 兜底级别用于兼容历史数据为空或异常枚举值的情况。
const defaultLevelMeta: { text: string; type: 'info' | 'warning' | 'danger' } = {
  text: '普通',
  type: 'info',
}

// 表单校验把明显的空值和长度问题前置，减少无效请求进入后端。

// 统计卡片从系统公告聚合接口读取，替换原通知总数/未读/已读/紧急通知。
const statCards = computed<StatCard[]>(() => [
  ...
  {
    title: '未读送达',
    value: String(stats.value.unreadDeliveryCount || 0),
    desc: '用户尚未阅读',
...
const fetchStats = async () => {
  const result = await getSystemAnnouncementStatistics()
...
const fetchRows = async (pageNum = currentPage.value, size = pageSize.value) => {
  ...
  try {
    ...
    const result = await listSystemAnnouncements({
...
const handleEdit = (row: SystemAnnouncement) => {
...
const handleSelectionChange = (selection: SystemAnnouncement[]) => {
...
const handleDelete = async (row: SystemAnnouncement) => {
  try {
...
const handleBatchDelete = async () => {
  if (!selectedRows.value.length) {
    ElMessage.warning('请先选择需要删除的系统公告')
    return
  }
  try {
    await ElMessageBox.confirm(`确定要批量删除已选中的 ${selectedRows.value.length} 条系统公告吗？`, '批量删除确认', {
      confirmButtonText: '确认删除',
      cancelButtonText: '取消',
      type: 'warning',
    })
    await batchDeleteSystemAnnouncements(selectedRows.value.map((row) => row.id))
...
// “发布通知”快捷入口进入页面后直接打开已有公告发布表单。
const applyHomeRouteIntent = () => {
  if (route.query.action === 'create') openPublishDialog()
}

watch(() => route.fullPath, applyHomeRouteIntent)
...
<template>
  <div class="notice-page admin-page">
    ...
    <div class="notice-shell admin-shell">
      <Header :breadcrumbs="['首页', '消息中心', '系统公告']" @toggle-sidebar="sidebarVisible = true" />
        ...
        <section class="filter-card">
          <el-form class="filter-form" :model="searchForm" label-width="76px" @submit.prevent="refreshPage(1)">
            ...
            <div class="manage-actions">
              ...
              <el-button type="primary" :icon="CirclePlus" @click="openPublishDialog">发布公告</el-button>
            </div>
          </el-form>
        </section>
        ...
        <section class="table-card">
          <div class="table-scroll">
            ...
            >
              ...
              <el-table-column prop="title" label="公告标题" min-width="220" fixed="left" show-overflow-tooltip />
...
.filter-form {
  ...
  align-items: center;
  gap: 14px 22px;
  /* 右侧预留管理按钮空间，使查询/重置与用户信息管理页面的按钮排布一致。 */
  padding-right: 250px;
}
