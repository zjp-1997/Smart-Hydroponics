 ...
 * 水泵监测数据业务实现类。
 *
 * <p>由于水泵监测数据由传感器自动上传，本服务仅提供删除、批量删除和查询功能，不提供新增和修改功能。</p>
 */
@Service
@RequiredArgsConstructor
public class PumpDataServiceImpl implements PumpDataService {
    ...
    public void deletePumpData(Long id) {
        requireId(id);
        PumpData data = getPumpDataById(id);
        dataPermissionService.requireFarmManager(data.getOwnerUserId());
        int rows = pumpDataMapper.deleteById(id);
        if (rows == 0) {
            throw new BusinessException(ResponseCode.NOT_FOUND, "水泵监测数据不存在");
        }
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public int deletePumpDataBatch(List<Long> ids) {
        if (ids == null || ids.isEmpty()) {
            throw new BusinessException(ResponseCode.PARAM_ERROR, "请选择要删除的水泵监测数据");
        }
        for (Long id : ids) {
            requireId(id);
            PumpData data = getPumpDataById(id);
        ...
        return pumpDataMapper.deleteBatchByIds(ids);
    }

    @Override
    public PumpData getPumpDataById(Long id) {
        requireId(id);
        PumpData data = pumpDataMapper.selectById(id);
        if (data == null) {
            throw new BusinessException(ResponseCode.NOT_FOUND, "水泵监测数据不存在");
        }
    ...
    @Override
    public PageInfo<PumpData> listPumpData(Long plotId, Long deviceId, String deviceCode, Integer dataStatus,
        ...
        return new PageInfo<>(pumpDataMapper.selectList(
                scopedUserId, plotId, deviceId, normalizeOptionalText(deviceCode), dataStatus, startTime, endTime));
    }

    @Override
    public Map<String, Object> statisticsPumpData(Long plotId, Long deviceId, String deviceCode,
                                                  Integer dataStatus, LocalDateTime startTime,
                                                  LocalDateTime endTime) {
        validateQueryParams(dataStatus, startTime, endTime);
        Long scopedUserId = dataPermissionService.restrictUserId(null);
        return pumpDataMapper.selectStatistics(
                scopedUserId, plotId, deviceId, normalizeOptionalText(deviceCode), dataStatus, startTime, endTime);
    }
    ...
    private void requireId(Long id) {
        if (id == null) {
            throw new BusinessException(ResponseCode.PARAM_ERROR, "水泵监测数据ID不能为空");
        }
    }
