	...
	<view class="picture-page">
		<view class="page-hero">
			<view class="navbar">
				...
				<text class="nav-title">{{ selectionMode ? `已选择${selectedCount}项` : '手机图片' }}</text>
...
<script>
import { deletePictures, downloadPicturesToAlbum, getPhonePictureList } from '@/api/pictureList.js'
...
export default {
	...
	methods: {
		...
		fetchPictures() {
			this.loading = true
			// 当前页面只展示手机图片，用户身份由请求头中的 token 在后端解析。
			return getPhonePictureList()
				.then((pictures) => {
		...
		async handleDownloadSelected() {
			...
			try {
				// 下载能力统一收敛到 api/pictureList.js，页面只传递当前选中的手机图片引用。
