    ...
    <scroll-view class="maintenance-content" scroll-y>
      ...
      <view v-else-if="!messages.length" class="state-card empty-state">
        <view class="empty-icon" aria-hidden="true">
          <text class="iconfont icon-xitonggonggao"></text>
        </view>
        <text class="empty-title">暂无系统消息</text>
        <text class="empty-tip">发布给您的系统公告会展示在这里</text>
      </view>
      ...
      <view v-else class="message-list">
        ...
        <view
          ...
          role="button"
          :aria-label="`${item.title || '系统公告'}，${item.readCount ? '已读' : '未读'}，点击查看详情`"
        ...
        >
          ...
          <view class="message-main">
            <text class="message-title">{{ item.title || '系统公告' }}</text>
      ...
      <view class="detail-dialog" @tap.stop>
        <view class="detail-head">
          ...
          <view class="detail-heading">
            <text class="detail-title">{{ selectedMessage.title || '系统公告' }}</text>
...
} from '@/api/systemAnnouncement.js'
  ...
  methods: {
    ...
    handleBack() {
      ...
      if (pages.length > 1) {
        ...
        // 无返回栈时依据角色回到对应消息入口，避免进入农场主 TabBar。
        const role = String(getUserInfo()?.roleCode || '').toLowerCase()
        if (role === 'technician') {
          uni.reLaunch({ url: '/pages/technician/news' })
          return
        }
        if (role === 'expert') {
        uni.reLaunch({ url: '/pages/expert/news' })
        return
      }
    ...
    async openMessage(item) {
      ...
      try {
        await markSystemAnnouncementRead(item.id)
        item.readCount = 1
